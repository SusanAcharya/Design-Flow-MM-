...
export function DiscoverScreen() {
  ...
  const exploreTools = [
    { title: "Stock screener", sub: "Sector, 52-week range, turnover", icon: "discover" as const, action: () => go("search") },
    { title: "Company analysis", sub: "Financials, ratios and events", icon: "market" as const, action: () => go("stock", { stock: "NABIL" }) },
    { title: "Compare companies", sub: "Same dates, same definitions", icon: "compare" as const, action: () => openSheet({ kind: "compare" }) },
    { title: "WACC calculator", sub: "Average cost and fee inputs", icon: "wallet" as const, action: () => openSheet({ kind: "metric", id: "wacc" }) },
    { title: "IPO tracker", sub: "Open issues and allotment status", icon: "cal" as const, action: () => go("ipo") },
    { title: "Alerts", sub: "Price, event, IPO and circuit", icon: "alert" as const, action: () => go("alerts") },
  ];
  return (
    <div>
      {viewport === "mobile" && (
      ...
      <div className="pad" style={{ paddingBottom: 12 }}>
        <button className="profile-row" onClick={() => openSheet({ kind: "profile" })}>
          <span className="avatar">SP</span>
          <span style={{ flex: 1 }}>
            <strong className="t-h-s">Sandip</strong>
            <small>{stageMeta[stage].label} · Profile and settings</small>
          </span>
      ...
      <div className="pad" style={{ paddingBottom: 12 }}>
        {viewport === "mobile" && (
          <SearchField placeholder="Search a company, a broker code, an IPO or a term" onFocus={() => go("search")} />
        )}
      </div>
      <div className="section-head"><span className="overline">Tools</span></div>
      <div className="tool-grid">
        {exploreTools.map((tool) => (
          <button key={tool.title} className="tool-tile" onClick={tool.action}>
            <span className="ico-soft"><Icon name={tool.icon} size={18} /></span>
            <strong className="t-h-s">{tool.title}</strong>
            <span className="t-body-xs muted">{tool.sub}</span>
          </button>
        ))}
      </div>
      <div className="gap-20" />
      <div className="pad"><Overline>Jump in</Overline></div>
      {discover.map((d) => (
        <button
          key={d.title}
          className="row"
          onClick={() => {
            if (d.kind === "Stock") go("stock", { stock: "NABIL" });
            else if (d.kind === "IPO") go("ipo");
            else if (d.kind === "Gyan") go("lesson", { lesson: d.title });
            else go("search");
          }}
        >
          <div className="row-main">
            <p className="t-label-s c-muted">{d.kind}</p>
            <p className="t-h-s">{d.title}</p>
            <p className="row-sub">{d.sub}</p>
          </div>
          <Icon name="chev" size={15} />
        </button>
      ))}
    ...
    <div>
      <div className="tulkey-ai-head">
        ...
        <div>
          ...
          <p className="t-body-s muted">Explanations and platform guidance — never a stock pick.</p>
        </div>
      </div>
      <div className="pad">
        <button className="ai-composer" type="button">
          <span>Ask “What is EDIS?” or “Where do I apply?”</span>
      ...
      {objective && (
        <>
          <div className="section-head"><span className="overline">Continue learning</span></div>
          <div className="pad">
            <button className="attention ai-objective" onClick={() => go("objective")}>
              <div className="kicker"><Icon name="learn" size={16} /> Your objective</div>
              <p className="t-h-s">{objective.title}</p>
              <p className="t-body-xs muted">{objective.duration} · educational</p>
            </button>
          </div>
        </>
      )}
      <div className="gap-16" />
      <div className="section-head"><span className="overline">Try asking</span></div>
      {["What is a kitta?", "How does an IPO application work?", "What is a limit order?", "Why did a price stop moving?"].map((q) => (
        <button key={q} className="row" onClick={() => go("lesson", { lesson: q })}>
          <span className="learn-ico"><Icon name="learn" size={17} /></span>
          <div className="row-main"><p className="t-h-s">{q}</p></div>
          <Icon name="chev" size={15} />
        </button>
      ))}
    </div>
